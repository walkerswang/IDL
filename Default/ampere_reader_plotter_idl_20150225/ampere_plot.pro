...
;
; Caveat:  The original routine was written using the VECTOR function to display the magnetic perturbations.
;          While extremely fast, in testing of the routine it was found that not all vectors are displayed 
;          correctly. The only feasible work around was to plot the vectors using the ARROW function which
;          is unfortunately very slow. I am working with ITTVIS to resolve the issue. In the meantime,
;          grab yourself a cup of coffee.
;                
; Additional requirements: The use of the show_globe keyword requires the IDL AACGM DLM v5.1 or above, 
;          available at http://dysprosium.jhuapl.edu/idl_aacgm.
; 
; Author:
; Haje Korth
; JHU/APL
...
pro ampere_plot,ncdfname,start_hr,start_mt,show_globe=show_globe
...
; add fitted dB plot
  if (hem_str eq 'north') then begin
    ...
    dbmap.refresh,/disable
    for i=0,n_elements(lon)-1 do begin
      if ((i mod 100) eq 0) then begin
        print,strtrim(string(i),2)+' / '+strtrim(string(n_elements(lon)),2)
        wait,0.01
      endif
      
      if (lat[i] ge latmin) then begin
        xy0=dbmap.mapforward(lon[i],lat[i])    
        uvn=[-xy0[0],-xy0[1]]
        uve=[-xy0[1],+xy0[0]]
        mg=sqrt(total(uvn^2))
        uvn=uvn/mg
        uve=uve/mg       
        x1=xy0[0]+(dbnorth1[i]*uvn[0]+dbeast2[i]*uve[0])*arrow_scl
        y1=xy0[1]+(dbnorth1[i]*uvn[1]+dbeast2[i]*uve[1])*arrow_scl
        tp=dbmap.mapinverse(x1,y1)       
        !null=arrow([lon[i],tp[0]],[lat[i],tp[1]],/data,head_size=0.1,head_indent=1,target='dbmap')
      endif
    endfor
    dbmap.refresh
    ...
    for i=0,n_elements(lon)-1 do begin
      ...
      if (lat[i] le -latmin) then begin
        xy0=dbmap.mapforward(360-lon[i],lat[i])    
        uvn=[+xy0[0],+xy0[1]]
        uve=[-xy0[1],+xy0[0]]
        mg=sqrt(total(uvn^2))
        uvn=uvn/mg
        ...
        y1=xy0[1]+(dbnorth1[i]*uvn[1]+dbeast2[i]*uve[1])*arrow_scl
        tp=dbmap.mapinverse(x1,y1)       
        !null=arrow([360-lon[i],tp[0]],[lat[i],tp[1]],/data,head_size=0.1,head_indent=1,target='dbmap')
      endif
    endfor
    dbmap.refresh
