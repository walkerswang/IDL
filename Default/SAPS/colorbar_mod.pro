...
; AUTHOR:
;
;    Slightly modified version of the Fanning colorbar.pro routine
;
...
;                          LoadCT, 33
;                          TVImage, image
;                          Colorbar_fc, Color=FSC_Color('firebrick')
;
;                     The proper syntax for device-independent color is like this:
;
;                          LoadCT, 33
;                          TVImage, image
;                          Colorbar_fc, AnnotateColor='firebrick', Color=255
;
...
;       COLORBAR_FC, NCOLORS=100, POSITION=[0.15, 0.85, 0.95, 0.90]
;
...
PRO COLORBAR_MOD, BOTTOM=bottom, CHARSIZE=charsize, COLOR=color, DIVISIONS=divisions, $
  ...
  IF KEYWORD_SET(vertical) THEN BEGIN
    ;--- BCW - modification to expand entries 0 and 255
    ;      bar = REPLICATE(1B,20) # BINDGEN(ncolors)

    bar = REPLICATE(1B,20) # (INDGEN(ncolors+2)-1)
    bar[*,0] = 0
    bar[*,ncolors+1] = (ncolors-1)
    bar = BYTE(bar)
  ...
  ENDIF ELSE BEGIN
    ...
    bar              = (INDGEN(ncolors+2)-1) # REPLICATE(1B,20)
    bar[0,*]         = 0
    bar[ncolors+1,*] = (ncolors-1)
    bar = BYTE(bar)
  ...
  ENDIF ELSE BEGIN
    ...
    ;--- BCW - modification to preserve entry 255
    ;      bar = CONGRID(bar, CEIL(xsize*!D.X_VSize), CEIL(ysize*!D.Y_VSize), /INTERP)
    bar = round(CONGRID(float(bar), CEIL(xsize*!D.X_VSize), CEIL(ysize*!D.Y_VSize), /INTERP))
