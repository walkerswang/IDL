...
allFILELIST = FINDFILE(FOLDER+'data/3DALL_t170907_230000.bin')
...
for ifile = ff, lf do begin
    ...
    for ii=0,n_elements(ltime)-1 do begin
        ...
        figfile= folder+'plot_new/meridional/ne'+strtrim(ltime[ii],2)+'_'+strmid(filename,tp+2,13)
        
        setdevice, figfile+'.ps'

        plot,lat,alt,/nodata,xstyle=1,ystyle=1,xcharsize=1.5,YTITLE='Altitude (km)',xticklen=-0.03,xticks=12,title=strmid(filename,tp+2,13),xtitle='Latitude'

        FOR k=0, 182 DO BEGIN;  UT
          x1=lat[k]
          x2=lat[k+1]
          x3=lat[k+1]
          x4=lat[k]
