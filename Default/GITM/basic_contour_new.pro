...
PRO basic_contour_new, data_array, maxrange, posarr, levels, 		$
	plotmaxmin = plotmaxmin, white = white, color=color

  if n_elements(plotmaxmin) eq 0 then pmm = 0				$
  else pmm = 1

  if n_elements(white) eq 0 then pw = 0					$
  else pw = 255
  ...
  for i = 0, nrad-1 do begin
	...
	polear(0:nazm-1,i) = ave
      ...
      xcor(0:nazm-1,i) = radvalues(loc)*sin(azmvalues(loc)*!pi/180.0)
      ycor(0:nazm-1,i) = radvalues(loc)*cos(azmvalues(loc)*!pi/180.0)
