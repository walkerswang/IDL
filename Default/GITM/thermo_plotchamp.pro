...
;    endif
    ...
    if (nFiles lt 10 and nFiles gt 1) then begin
       spawn, "ls -s "+filelist(0), s
       s = long(s(0))
       if (s gt 100) then begin
          print, "Less than 10 files found.  They also seem sort of big."
          print, "Assuming that they are appended files."
          print, "If I am wrong, enter -1."
          display, filelist
          if (n_elements(iFile) eq 0) then iFile = 0
          iFile = fix(ask('which file you would like to plot',tostr(iFile)))
          if (iFile ge 0) then filelist = filelist(iFile)
       endif
    endif
    ...
    if (min(dt) lt 0.0) then begin
       ...
       for i=0,nTimes-1 do datafixed(0,i,*,*) = data(0,order(i),*,*)
       data = datafixed
    endif    
    ...
    if plotdiff then begin
        thermo_readsat, filelist_base, data_base, time_base, nTimes_base, $
                        Vars_base, nAlts_base,$
    ...
    c_r_to_a, itime, time(0)
    itime(3:5) = 0
    c_r_to_a, itimes, time(0)
    itimes(3:5) = 0
    c_a_to_r, itimes, stimeg
    c_r_to_a, itimee, time(nTimes-1)
    itimee(3:5) = [23,59,59]
    c_a_to_r, itimee, etimeg
    print, etimeg-stimeg, (etimeg-stimeg)/3600.0

    ndays = fix((etimeg-stimeg)/3600. /24.)+1    
   ...
   ;     champ_file_w = champdir+'Wind_3deg_'+tostr(iYear mod 100,2)+'_'+tostr(doy)+'.ascii'
...
;        ChampLTs(iTime)       = ChampLocalTime(i)
        ...
        GitmeDensity(iTime) = (1.0 - x) * GitmElectron(iTime,i-1) + $
          (      x) * GitmElectron(iTIme,i)
