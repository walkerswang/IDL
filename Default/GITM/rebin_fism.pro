...
idate = [year,month,day,0,0,0]
c_a_to_r, idate,rt
cdoy = chopr('00'+tostr(jday(year,month,day)),3)

rootdir = '/bigdisk1/Data/FISM/'

fismfile = file_search(rootdir+cyear+'/FISM*60sec*_'+cyear+cdoy+'_*.sav')
fismfile = fismfile(0)

if strpos(fismfile,'.sav') ge 0 then begin
   restore,fismfile
   ;newfile = rootdir+'BinnedFiles/'+cyear+'/fismflux'+cyear+cmon+cday+'.dat'
   newfile = 'fismflux'+cyear+cmon+cday+'.dat'


;read in GITM wavelength information
    
   close,/all
   openr,30, rootdir+'wavelow'
   openr,31,rootdir+'wavehigh'
   nbins = 59
