...
pro contour_circle_nick, data, lons, lats, $
  ...
  if (count gt 0) then begin
     ...
     if (not nolines) then begin
        ...
        contour, data(*,loc), x(*,loc), y(*,loc), /noerase, pos = pos, $
               levels = levels, $
                 ...
                 xrange = [-maxrange,maxrange], $
                 yrange = [-maxrange,maxrange], thick = thick,c_charsize=1.3
     endif else begin
     ...
     if (average) then begin
      ...
      if (not nomaxmin) then begin
        ...
        ; xyouts, p1, p2, mins, /norm, charsize = 2.0, orient = -45, alignment=0.5

         p1 = pos(2)-(pos(2) - pos(0))*0.30 * (1.0 - sin(45.0*!pi/180.0))*0.95
         ;xyouts, p1, p2, maxs, /norm, align=0.5, charsize = 2.0, $
          ;       orient = 45.0
      endif
      if (n_elements(title) gt 0) then begin
          ...
          xyouts, p1, p2, title, $
                /norm, alignment = 0.5, charsize = 2.0, orient = 45.0
      ...
      if (n_elements(colorbar) gt 0) then begin
         ...
         p1 = pos(2)-(pos(2) - pos(0))
          p2 = pos(3)-(pos(3) - pos(1))
          p1 = pos(2) - pos(0)
          p2 = pos(3) - pos(1)

          ctpos = [p1,p2*.3,p1+0.3,pos(3)]
          ctpos = [0.22,0.05,0.29,0.9]
          ;plotct_nick, 254, ctpos, [mini,maxi], colorbar, /right
