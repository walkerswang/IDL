;+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
;
;	       DMSP Horizontal Velocities (Portrait)
;
;+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++


PRO Type_19_Plot, single, t, dt, times, moncheck, seconds, same, basetime, titlenam, unitnam,    $
  ...
  if place eq 2 then begin
    ...
    pole = 'Northern Summer'
    ml = '60!Uo'

    xyouts, 47.0, 46.0, pole, alignment=1.0
    xyouts, 30.0, 30.0, ml
    xyouts, 0.0,-45, '00', alignment=0.5
  ...
  while (numofpts(val)-1 gt curpt) do begin
    ...
    if single(rad,curpt) ne 0.0 then sgn = abs(single(rad2,curpt))/single(rad2,curpt) else sgn = 1.0
	...
	oplot, px+[cirsiz*cos(a)], py+[cirsiz*sin(a)], linestyle = linsty
      ...
      if (abs(single(rad,curpt)) gt 50.0) then distold = dist
      ...
      if abs(plat) ge (minlat) then oplot, [px,nx], [py,ny]
    ...
    if (curpt eq numofpts(val)-1) or (place eq 1) then begin
      ...
      xyouts, 0.5, 0.98, datetl, charsize=1.5, /norm, alignment = 0.5
