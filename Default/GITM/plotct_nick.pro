...
pro plotct_nick, ncolors, pos, maxmin, title, right=right, 		$
    ...
    if (bottom) then begin
        ...
        if not right then begin
            plot, maxmin2, /noerase, pos = pos, 			$
              ...
              color = color_in,charsize=2.5
        endif else begin
            plot, maxmin2, /noerase, pos = pos, 			$
              xstyle=5,ystyle=5, /nodata,charsize=2.5
            if (pos(3)-pos(1) lt 0.1) then yt = 1 else yt = 0
            axis, 1, ystyle=1, /nodata, ytitle = title, yax=1, 	$
              charsize=3.0, color = color_in, yticks = yt
        endelse
    endelse
