...
pro convert_mhd_2d_to_save, filelist
  ...
  if (n eq 1) and (strlen(list(0)) eq 0) then begin
	...
	file = strmid(file, 0, s)
      ...
      openr,1,file
      n = 0
      while (n_equals lt 0) or (e_plus lt 0) do begin
        readf,1,line
        npos = strpos(line,"N=")
        epos = strpos(line,"E+")
	...
	n = n + 1
      ...
      openr,1,file
      for n=0,e_plus-1 do begin
	readf,1,line
      endfor

      print, 'Reading Data.'
      for n=0,nrows-1 do begin
        readf,1,tmp
	data(*,n) = tmp
      endfor
