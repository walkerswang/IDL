pro read_flat_vector, stime, etime, col, time, data, nrows,	$
                      filenum = filenum,			$
                      filename = filename
...
; set up data array
  ...
  for i = 0, nvar-1 do 					$
    if header.nele(id(i),col(i),0) gt max_y then 	$
      max_y = header.nele(id(i),col(i),0)

  data = fltarr(nvar, max_nrow, max_y)
  ...
  for i=0,nvar-1 do begin
    ...
    for j=start_row,end_row do begin
      ...
      fdget_vector, id(i), j, col(i), temptime, tempdata

      time(i,j-start_row) = temptime
      data(i,j-start_row,0:header.nele(id(i),col(i),0)-1) = tempdata
