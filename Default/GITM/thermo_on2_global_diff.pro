...
FILELIST = FINDFILE(FOLDER+'data/'+'3DALL_t170908_050000.bin')
FILELIST2 = FINDFILE(FOLDER+'data/'+'3DALL_t170907_050000.bin')
...
for ifile = ff, lf do begin
  ...
  on2file= folder+'plot_new/on2_global/'+strmid(file,31,20)+'on2_global_diff'
  ...
  if (count gt 0) then ratio1(loc) = o(loc)/n2(loc)
  ...
  for iLon = 0, nLons-1 do begin
    for iLat = 0, nLats-1 do begin
      ...
      noInt(iLon,iLat) = total(data2(ino_,iLon,iLat,2+loc_150)*dAlt_150(loc_150))

      iAlt = nAlts-1
      Done = 0
      if (max(data2(in2_,iLon,iLat,*)) eq 0.0) then Done = 1
      while (Done eq 0) do begin
        dAlt = (Alt(iAlt)-Alt(iAlt-1))
        n2Mid = (data2(in2_,iLon,iLat,iAlt) + $
          data2(in2_,iLon,iLat,iAlt-1)) /2.0
        oMid  = (data2(io_,iLon,iLat,iAlt) + $
          data2(io_,iLon,iLat,iAlt-1)) /2.0
  ...
  ratio2 = n2*0.0
  loc = where(n2 gt 0.0,count)
  if (count gt 0) then ratio2(loc) = o(loc)/n2(loc)
  loc = where(n2 eq 0.0,count)
