...
;        n = fix(strmid(cFile,5,3))
    ...
    for i=0,nFiles-1 do begin
       ...
       IU = fix((float(i)*100.0)/nFiles)
       IL = fix((float(i-1)*100.0)/nFiles)
       if (iU ne iL) then print, IU,'% done'
       ...
       if (strpos(cFile, sats(isat)) eq 0) then begin
          ...
          if (s(0) eq 2) then begin
             ; This means that we have more than one time
             ; stored in the file!
             nTimes = s(1)
          endif
          ...
          if (s(0) eq 1) then begin
             ...
             for j=0,nTimes-1 do begin
                data(n,j,*,*) = data_t(j,*,0,0,*)
                c_a_to_r, reform(itime(j,*)), t
                time(j) = t
             endfor
          endelse
