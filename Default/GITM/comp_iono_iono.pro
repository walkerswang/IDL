...
if (nfiles ne 2) then begin
  print, "Hey, you to enter 2 files here."
  stop
endif
...
for n = 0, nfiles-1 do begin
  ...
  if strpos(filein,'save') gt -1 then begin
	...
	stop
      ...
      data_to_plot = fltarr(2,nfiles+1,nvars_to_plot,nlons,nlats)
...
toff = 0.0
if (strpos(relative,'r') eq 0) then toff = time(0) - dt - 24.0*3600.0
time = time - toff
...
for hem = 0, isouth do begin
  ...
  for i = 0, nvars_to_plot-1 do begin
    ...
    data_to_plot(hem,0,i,*,*) = data_to_plot(hem,0,i,*,*) - data_to_plot(hem,1,i,*,*)

    mini = min(data_to_plot(hem,0,i,*,loc))
    maxi = max(data_to_plot(hem,0,i,*,loc))
