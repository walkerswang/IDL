pro thermo_plot_nick,cursor_x,cursor_y,strx,stry,step,nvars,sel,nfiles, $
...
;    endelse
    ...
    if (cnt1) then begin
        ...
        contour,d2, x2, y2,POS=pos,$
          ...
          xrange=xrange,yrange=yrange,$
          c_colors=clevels,max_value=25,min_value=-25,$
    ...
    if (polar) then begin
        plotmlt_nick, mr,/no18,/no06,/no12
...
;           print, northpolelat, southpolelat
    ...
    southpolelat = -76.5
    southpolelon = 110.5
   ; northpolelon = 3.0979594
   ; northpolelat = 1.3235413
   ; southpolelat = -1.0908308
   ; southpolelon = 2.2252948
           if (npolar eq 1) then begin
...
;            print, polerange
            ...
            print, 'NPX AND NPY'
            print, npx*180/!pi, npy*180/!pi
            print, polelon
            print, southpolelon
            print, northpolelon
            print, polerange
            oplot, [npx],[npy], psym = 4, symsize = 3, thick = 20
       ; endif
...
;    utrot = 180.0
    ...
    thermo_plotvectors_nick,vars,selset,data,lat, $
      lon, utrot, alt, $
...
title = variable+' [K]'
plotct_nick,254,pos,maxmin,title,/right,color=color
