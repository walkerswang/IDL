...
filelist = file_search(foldername+'/data/3DALL_t17090*.bin')
file = filelist(0)
...
setdevice, foldername+'/plot_new/tec_hemisphere.ps','p',5, 0.9
...
save,tmptime, tecnorthday,tecsouthday, filename=foldername+'/plot_new/tecday.sav'
...
if (DoWriteFile) then begin
   ...
   outfile=foldername+'/plot/tec_summary_'+ymd+'.txt'
   openw,1,outfile
...
AverageAll =0 ;fix(ask('Average all times together (0=no,1=yes)',tostr(AverageAll)))
...
maxi =30 ;float(ask('enter maximum value for global plot',tostr(maxi)))

for iFile = 0, nFiles-1 do begin
   file = filelist(iFile)
  
   setdevice, foldername+'/plot_new/tec_new/tec_'+strmid(file,38,13)+'.ps', 'p', 5 
