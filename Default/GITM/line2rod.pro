...
if (nZones gt 0) then begin
    ...
    print, "Checking to see if file is output file"

    ; Get number of points to read
    nStart=lonarr(nZones)
    nEnd=lonarr(nZones)

    if (strpos(sarr(0),"Satellite data") eq 0) then begin
        sFileType = "output"
        nStart(0) = 3
        nEnd(0) = nLines
        print, "File is an output type"
    endif else begin
        print, "File is not output type"
        for i=0L,10 do begin
            if (strpos(sarr(i),"START") gt -1) then begin
                sFileType = "input"
                nStart(0) = i+2
                nEnd(0) = nLines
                print, "File is an input type"
            endif
        endfor
        if (sFileType eq "unknown") then begin
            print, "Can not figure out file type.  Here is header:"
            for i=0L,10 do print, sarr(i)
            stop
        endif
    endelse
endelse
...
for z=0L,nZones-1 do begin
  ...
  if (sFileType eq "tecplot") then readf,1,xyzR
  if (sFileType eq "output" or sFileType eq "input") then begin
      itime = intarr(7)
      xyz_tmp = fltarr(3)
      if (sFileType eq "output") then begin
          itr = 0L
          for i=nStart(z), nEnd(z) do begin
              readf,1,itr,itime, xyz_tmp
              xyzR(*,i-nStart(z)) = xyz_tmp
          endfor
      endif else begin
          for i=nStart(z), nEnd(z) do begin
              readf,1,itime, xyz_tmp
              xyzR(*,i-nStart(z)) = xyz_tmp
          endfor
      endelse
  endif
