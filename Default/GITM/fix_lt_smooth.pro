...
for ii=0,n_elements(ltime)-1 do begin
    ...
    figfile= folder+'plot_new/meridional/ne_lt_north_smooth'+strtrim(ltime[ii],2)+'_'+strmid(filename,tp+2,13)

    setdevice, figfile+'.ps'

    plot,lat[121:182],alt,/nodata,xstyle=1,ystyle=1,xcharsize=1.5,YTITLE='Altitude (km)',xticklen=-0.03,xticks=18,title=strmid(filename,tp+2,13)

    FOR k=122, 181 DO BEGIN;  UT
      x1=lat[k]
      x2=lat[k+1]
      x3=lat[k+1]
      x4=lat[k]
    ...
    tickn=['-10!U11','-5x10!U10','0','5x10!U10','10!U11']
    num_level=256
    nvl=num_level
    ;!p.position=0.
