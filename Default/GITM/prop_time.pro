pro prop_time, pos, b, v, rho, pos2, title = title, psfile = psfile
...
;    if we assume that x_bowshock = x_mp/3.0
  ...
  plot, [-10,xmax],[ymin,ymax], /nodata, xstyle=1, ystyle=1, 	$
	pos = [0.05,0.05, 0.75, 0.95], xtitle = 'Re', 		$
  ...
  if s2 then add = ' 1' else add = ''
  xyouts, x/r0+1.0, y/r0+1.0, 'S/C'+add
  if s2 then begin
    oplot, [x2/r0], [y2/r0], psym = 4
    xyouts, x2/r0+1.0, y2/r0+1.0, 'S/C 2'
  endif
  ...
  xyouts, xmin/2 + xr/20.0 + xr/14.0, yout, 'IMF', charsize=1.2
  ...
  xyouts, 0.76, 0.9, 'Travel Times',/norm, charsize=1.2
  ...
  if s2 then begin
    ...
    xyouts, 0.78, 0.59, 'S/C #2 : '+smm+sss+' sec',/norm
  endif
  ...
  xyouts, 0.78, 0.52, 'Subsolar : '+smm+sss+' sec',/norm
  ...
  xyouts, 0.76, 0.25, 'IMF (Bx, By, Bz)', /norm, charsize=1.2
  imf = strcompress('('+strmid(tostrf(bx),0,5)+','+		$
			strmid(tostrf(by),0,5)+','+		$
			strmid(tostrf(bz),0,5)+')',/remove_all)
  xyouts, 0.78, 0.22, imf, /norm

  xyouts, 0.76, 0.15, 'Velocity (Vx, Vy, Vz)', /norm, charsize=1.2
  vel = strcompress('('+strmid(tostrf(vx),0,6)+','+		$
			strmid(tostrf(vy),0,6)+','+		$
			strmid(tostrf(vz),0,6)+')',/remove_all)
  xyouts, 0.78, 0.12, vel, /norm

  srho = strmid(tostrf(rho),0,4)
  xyouts, 0.76, 0.05, 'Rho : '+srho, /norm, charsize=1.2
