...
;   $URL: https://ergsc-local.isee.nagoya-u.ac.jp/svn/ergsc/trunk/erg/ground/geomag/erg_load_gmag_isee_fluxgate.pro $
;-
    ...
    if(total(filestest) ge 1) then begin
      ...
      if(downloadonly eq 0) then begin
        cdf2tplot, file=files, verbose=source.verbose, $
                   prefix='isee_fluxgate_', suffix='_'+site_code[i], varformat='*hdz_'+tres[where(tres[j:j*2] ne '')+j]+'*'

        for k=j, j*2 do begin
          if(tres[k] ne '') then begin
            ;--- Rename
            if(tnames('isee_fluxgate_mag_'+site_code[i]+'_'+tres[k]+'_hdz') eq 'isee_fluxgate_mag_'+site_code[i]+'_'+tres[k]+'_hdz') then $
              del_data, 'isee_fluxgate_mag_'+site_code[i]+'_'+tres[k]+'_hdz'
            store_data, 'isee_fluxgate_hdz_'+tres[k]+'_'+site_code[i], newname='isee_fluxgate_mag_'+site_code[i]+'_'+tres[k]+'_hdz'

            ;--- time clip
            if(keyword_set(timeclip)) then begin
              get_timespan, tr & tmspan=time_string(tr)
              time_clip, 'isee_fluxgate_mag_'+site_code[i]+'_'+tres[k]+'_hdz', tmspan[0], tmspan[1], /replace
            endif

            ;--- Missing data -1.e+31 --> NaN
            tclip, 'isee_fluxgate_mag_'+site_code[i]+'_'+tres[k]+'_hdz', -1e+4, 1e+4, /overwrite

            ;--- Labels
            options, 'isee_fluxgate_mag_'+site_code[i]+'_'+tres[k]+'_hdz', labels=['H','D','Z'], labflag=1, colors=[2,4,6]
